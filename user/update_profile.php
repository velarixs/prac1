...
if(isset($_POST['submit'])){
   ...
   if(!empty($first_name)){
      ...
      $update_first_name->execute([$first_name, $user_id]);
   }

   if(!empty($last_name)){
      $update_last_name = $conn->prepare("UPDATE `users` SET last_name = ? WHERE id = ?");
      $update_last_name->execute([$last_name, $user_id]);
   }
...
<section class="form-container">
   ...
   <form action="" method="post">
      ...
      <?php if($fetch_profile['image'] != ''){ ?>
         <img src="../uploaded_img/<?= $fetch_profile['image']; ?>" class="image" alt="">
      <?php } ?>
      </div>

      <input type="text" name="first_name" placeholder="<?= $fetch_profile['first_name']; ?>" class="box" maxlength="50">
      <input type="text" name="last_name" placeholder="<?= $fetch_profile['last_name']; ?>" class="box" maxlength="50">
